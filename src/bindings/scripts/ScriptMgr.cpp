/* Copyright (C) 2006 - 2008 QuadScript <https://scriptdev2.svn.sourceforge.net/>
...
#define _FULLVERSION "QuadScript"

#ifndef _QUAD_SCRIPT_CONFIG
# define _QUAD_SCRIPT_CONFIG  "quadcore.conf"
#endif _QUAD_SCRIPT_CONFIG
...
void LoadDatabase()
{
    ...
    if (dbstring.empty())
    {
        error_log("QUADSCRIPT: Missing world database info from configuration file. Load database aborted.");
        return;
    }
    ...
    {
        error_log("QUADSCRIPT: Unable to connect to database at %s. Load database aborted.", dbstring.c_str());
        return;
    }
...
outstring_log("                                                            @@         ");
outstring_log("_________________________________________________________________________");	
    outstring_log("Quad Script initializing %s", _FULLVERSION);
    outstring_log("");
    ...
    outstring_log("QUADSCRIPT: Loading C++ scripts");
    barGoLink bar(1);
    bar.step();
    outstring_log("");
...
void DoScriptText(int32 iTextEntry, WorldObject* pSource, Unit* pTarget)
{
    if (!pSource)
    {
        error_log("QUADSCRIPT: DoScriptText entry %i, invalid Source pointer.", iTextEntry);
        return;
    }

    if (iTextEntry >= 0)
    {
        error_log("QUADSCRIPT: DoScriptText with source entry %u (TypeId=%u, guid=%u) attempts to process text entry %i, but text entry must be negative.", pSource->GetEntry(), pSource->GetTypeId(), pSource->GetGUIDLow(), iTextEntry);
        return;
    }
    ...
    if (!pData)
    {
        error_log("QUADSCRIPT: DoScriptText with source entry %u (TypeId=%u, guid=%u) could not find text entry %i.", pSource->GetEntry(), pSource->GetTypeId(), pSource->GetGUIDLow(), iTextEntry);
        return;
    }

    debug_log("QUADSCRIPT: DoScriptText: text entry=%i, Sound=%u, Type=%u, Language=%u, Emote=%u", iTextEntry, pData->uiSoundId, pData->uiType, pData->uiLanguage, pData->uiEmote);
    ...
    if(pData->uiSoundId)
    {
        if(GetSoundEntriesStore()->LookupEntry(pData->uiSoundId))
        {
            ...
            error_log("QUADSCRIPT: DoScriptText entry %i tried to process invalid sound id %u.", iTextEntry, pData->uiSoundId);
    }
    ...
    if(pData->uiEmote)
    {
        if (pSource->GetTypeId() == TYPEID_UNIT || pSource->GetTypeId() == TYPEID_PLAYER)
            ...
            error_log("QUADSCRIPT: DoScriptText entry %i tried to process emote for invalid TypeId (%u).", iTextEntry, pSource->GetTypeId());
    }
...
void Script::RegisterSelf()
{
    ...
    {
        error_db_log("CRASH ALERT! QuadScript: RegisterSelf, but script named %s does not have ScriptName assigned in database.",(this)->Name.c_str());
        delete this;
    }
}
...
{
    return "Default Quad scripting library";
}
...
{
    debug_log("QUADSCRIPT: Gossip selection, sender: %d, action: %d", uiSender, uiAction);
...
{
    debug_log("QUADSCRIPT: Gossip selection with code, sender: %d, action: %d", uiSender, uiAction);
