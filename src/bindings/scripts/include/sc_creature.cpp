...
void ScriptedAI::DoPlaySoundToSet(WorldObject* pSource, uint32 uiSoundId)
{
    ...
    {
        error_log("QUADSCRIPT: Invalid soundId %u used in DoPlaySoundToSet (Source: TypeId %u, GUID %u)", uiSoundId, pSource->GetTypeId(), pSource->GetGUIDLow());
        return;
    }
...
void ScriptedAI::DoResetThreat()
{
    ...
    {
        error_log("QUADSCRIPT: DoResetThreat called for creature that either cannot have threat list or has empty threat list (m_creature entry = %d)", m_creature->GetEntry());
        return;
    }
...
void ScriptedAI::DoTeleportPlayer(Unit* pUnit, float fX, float fY, float fZ, float fO)
{
    ...
    {
        if(pUnit)
            error_log("QUADSCRIPT: Creature %u (Entry: %u) Tried to teleport non-player unit (Type: %u GUID: %u) to x: %f y:%f z: %f o: %f. Aborted.", m_creature->GetGUID(), m_creature->GetEntry(), pUnit->GetTypeId(), pUnit->GetGUID(), fX, fY, fZ, fO);
        return;
    }
...
{
    Unit* pUnit = NULL;
    Quad::MostHPMissingInRange u_check(m_creature, fRange, uiMinHPDiff);
...
std::list<Creature*> ScriptedAI::DoFindFriendlyCC(float fRange)
{
    ...
    Quad::FriendlyCCedInRange u_check(m_creature, fRange);
...
{
    std::list<Creature*> pList;
    Quad::FriendlyMissingBuffInRange u_check(m_creature, fRange, uiSpellid);
...
Player* ScriptedAI::GetPlayerAtMinimumRange(float fMinimumRange)
{
    ...
    CellPair pair(Quad::ComputeCellPair(m_creature->GetPositionX(), m_creature->GetPositionY()));
    Cell cell(pair);
    cell.data.Part.reserved = ALL_DISTRICT;
    cell.SetNoCreate();

    Quad::PlayerAtMinimumRangeAway check(m_creature, fMinimumRange);
    Quad::PlayerSearcher<Quad::PlayerAtMinimumRangeAway> searcher(m_creature, pPlayer, check);
    TypeContainerVisitor<Quad::PlayerSearcher<Quad::PlayerAtMinimumRangeAway>, GridTypeMapContainer> visitor(searcher);
...
bool ScriptedAI::EnterEvadeIfOutOfCombatArea(const uint32 uiDiff)
{
    ...
    switch(m_creature->GetEntry())
    {
        ...
        case NPC_SARTHARION:                                         // sartharion (calculate box)
            ...
            break;
        default:
            error_log("QUADSCRIPT: EnterEvadeIfOutOfCombatArea used for creature entry %u, but does not have any definition.", m_creature->GetEntry());
            return false;
    }
