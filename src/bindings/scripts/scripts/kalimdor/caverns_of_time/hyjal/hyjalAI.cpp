...
void hyjalAI::SummonNextWave(Wave wave[18], uint32 Count, float Base[4][3])
{
    ...
    if (!wave[Count].IsBoss)
    {
        ...
        {
            NextWaveTimer = 15000;
            debug_log("QUADSCRIPT: HyjalAI: debug mode is enabled. Next Wave in 15 seconds");
        }
    }
    else
    {
...
void hyjalAI::HideNearPos(float x, float y)
{
    ...
    Quad::AllFriendlyCreaturesInGrid creature_check(m_creature);
    Quad::CreatureListSearcher<Quad::AllFriendlyCreaturesInGrid> creature_searcher(m_creature, creatures, creature_check);
    TypeContainerVisitor
        <Quad::CreatureListSearcher<Quad::AllFriendlyCreaturesInGrid>,
...
{
    CellPair p(Quad::ComputeCellPair(x, y));
    Cell cell(p);
    cell.data.Part.reserved = ALL_DISTRICT;
    cell.SetNoCreate();

    Quad::RespawnDo u_do;
    Quad::WorldObjectWorker<Quad::RespawnDo> worker(m_creature, u_do);
