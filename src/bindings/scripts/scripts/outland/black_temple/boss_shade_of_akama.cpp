...
struct QUAD_DLL_DECL mob_ashtongue_channelerAI : public ScriptedAI
{
...
struct QUAD_DLL_DECL mob_ashtongue_sorcererAI : public ScriptedAI
{
...
struct QUAD_DLL_DECL boss_shade_of_akamaAI : public ScriptedAI
{
    ...
    void IncrementDeathCount(uint64 guid = 0)               // If guid is set, will remove it from list of sorcerer
    {
        ...
        debug_log("QUADSCRIPT: Increasing Death Count for Shade of Akama encounter");
        ++DeathCount;
    ...
    void FindChannelers()
    {
        ...
        if (!ChannelerList.empty())
        {
            for (std::list<Creature*>::iterator itr = ChannelerList.begin(); itr != ChannelerList.end(); ++itr)
            {
                ...
                debug_log("QUADSCRIPT: Shade of Akama Grid Search found channeler %u. Adding to list", (*itr)->GetGUID());
            }
        }
...
struct QUAD_DLL_DECL npc_akamaAI : public ScriptedAI
{
