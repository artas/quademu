...
struct QUAD_DLL_DECL npc_manaforge_control_consoleAI : public ScriptedAI
{
...
//TODO: clean up this workaround when Quad adds support to do it properly (with gossip selections instead of instant summon)
...
struct QUAD_DLL_DECL npc_commander_dawnforgeAI : public ScriptedAI
{
    ...
    bool CanStartEvent(Player* pPlayer)
    {
        ...
        debug_log("QUADSCRIPT: npc_commander_dawnforge event already in progress, need to wait.");
        return false;
    }
...
struct QUAD_DLL_DECL mob_phase_hunterAI : public ScriptedAI
{
...
struct QUAD_DLL_DECL npc_bessyAI : public npc_escortAI
{
