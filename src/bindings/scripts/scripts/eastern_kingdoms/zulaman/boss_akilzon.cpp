...
struct QUAD_DLL_DECL boss_akilzonAI : public ScriptedAI
{
    ...
    void HandleStormSequence(Unit *Cloud) // 1: begin, 2-9: tick, 10: end
    {
        if (StormCount < 10 && StormCount > 1)
        {
            ...
            CellPair p(Quad::ComputeCellPair(m_creature->GetPositionX(), m_creature->GetPositionY()));
            Cell cell(p);
            ...
            {
                Quad::AnyAoETargetUnitInObjectRangeCheck u_check(m_creature, m_creature, 999);
                Quad::UnitListSearcher<Quad::AnyAoETargetUnitInObjectRangeCheck> searcher(m_creature, tempUnitMap, u_check);

                TypeContainerVisitor<Quad::UnitListSearcher<Quad::AnyAoETargetUnitInObjectRangeCheck>, WorldTypeMapContainer > world_unit_searcher(searcher);
                TypeContainerVisitor<Quad::UnitListSearcher<Quad::AnyAoETargetUnitInObjectRangeCheck>, GridTypeMapContainer >  grid_unit_searcher(searcher);
...
struct QUAD_DLL_DECL mob_soaring_eagleAI : public ScriptedAI
{
