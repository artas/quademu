...
struct QUAD_DLL_DECL boss_selin_fireheartAI : public ScriptedAI
{
    boss_selin_fireheartAI(Creature* c) : ScriptedAI(c)
    {
        ...
        if (pInstance)
        {
            ...
            for (uint8 i = 0; i < size; ++i)
            {
                ...
                debug_log("QUADSCRIPT: Selin: Adding Fel Crystal %u to list", guid);
    ...
    void MovementInform(uint32 type, uint32 id)
    {
        if (type == POINT_MOTION_TYPE && id == 1)
        {
            ...
            else
            {
                ...
                error_log("QUADSCRIPT: Selin Fireheart unable to drain crystal as the crystal is either dead or despawned");
                DrainingCrystal = false;
            }
        }
    }
...
struct QUAD_DLL_DECL mob_fel_crystalAI : public ScriptedAI
{
