 ...
 * 
 *
 * Thanks to the original authors: MaNGOS <http://getcore.com/>
 *
...
void SystemMgr::LoadVersion()
{
    ...
    if (pResult)
    {
        ...
        outstring_log("QUADSCRIPT: Database version is: %s", pFields[0].GetString());
        outstring_log("");
    }
    else
    {
        error_log("QUADSCRIPT: Missing `version`.`script_version` information.");
        outstring_log("");
    }
}
...
void SystemMgr::LoadScriptTexts()
{
    ...
    if (pResult)
    {
        ...
        do
        {
            ...
            if (iId >= 0)
            {
                error_db_log("QUADSCRIPT: Entry %i in table `script_texts` is not a negative value.", iId);
                continue;
            }

            if (iId > TEXT_SOURCE_RANGE || iId <= TEXT_SOURCE_RANGE*2)
            {
                error_db_log("QUADSCRIPT: Entry %i in table `script_texts` is out of accepted entry range for table.", iId);
                continue;
            }
            ...
            if (pTemp.uiSoundId)
            {
                if (!GetSoundEntriesStore()->LookupEntry(pTemp.uiSoundId))
                    error_db_log("QUADSCRIPT: Entry %i in table `script_texts` has soundId %u but sound does not exist.", iId, pTemp.uiSoundId);
            }

            if (!GetLanguageDescByID(pTemp.uiLanguage))
                error_db_log("QUADSCRIPT: Entry %i in table `script_texts` using Language %u but Language does not exist.", iId, pTemp.uiLanguage);

            if (pTemp.uiType > CHAT_TYPE_ZONE_YELL)
                error_db_log("QUADSCRIPT: Entry %i in table `script_texts` has Type %u but this Chat Type does not exist.", iId, pTemp.uiType);
...
void SystemMgr::LoadScriptTextsCustom()
{
    ...
    if (pResult)
    {
        ...
        do
        {
            ...
            if (iId >= 0)
            {
                error_db_log("QUADSCRIPT: Entry %i in table `custom_texts` is not a negative value.", iId);
                continue;
            }

            if (iId > TEXT_SOURCE_RANGE*2 || iId <= TEXT_SOURCE_RANGE*3)
            {
                error_db_log("QUADSCRIPT: Entry %i in table `custom_texts` is out of accepted entry range for table.", iId);
                continue;
            }
            ...
            {
                if (!GetSoundEntriesStore()->LookupEntry(pTemp.uiSoundId))
                    error_db_log("QUADSCRIPT: Entry %i in table `custom_texts` has soundId %u but sound does not exist.", iId, pTemp.uiSoundId);
            }

            if (!GetLanguageDescByID(pTemp.uiLanguage))
                error_db_log("QUADSCRIPT: Entry %i in table `custom_texts` using Language %u but Language does not exist.", iId, pTemp.uiLanguage);

            if (pTemp.uiType > CHAT_TYPE_ZONE_YELL)
                error_db_log("QUADSCRIPT: Entry %i in table `custom_texts` has Type %u but this Chat Type does not exist.", iId, pTemp.uiType);
...
void SystemMgr::LoadScriptWaypoints()
{
    ...
    if (pResult)
    {
        ...
        do
        {
            ...
            if (!pCInfo)
            {
                error_db_log("QUADSCRIPT: DB table script_waypoint has waypoint for non-existant creature entry %u", pTemp.uiCreatureEntry);
                continue;
            }

            if (!pCInfo->ScriptID)
                error_db_log("QUADSCRIPT: DB table script_waypoint has waypoint for creature entry %u, but creature does not have ScriptName defined and then useless.", pTemp.uiCreatureEntry);
