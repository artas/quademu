...
class SqlQuery : public SqlOperation
{
    private:
        const char *m_sql;
        Quad::IQueryCallback * m_callback;
        SqlResultQueue * m_queue;
    public:
        SqlQuery(const char *sql, Quad::IQueryCallback * callback, SqlResultQueue * queue)
...
class SqlQueryHolder
{
    ...
    public:
        ...
        bool Execute(Quad::IQueryCallback * callback, SqlDelayThread *thread, SqlResultQueue *queue);
};
...
class SqlQueryHolderEx : public SqlOperation
{
    private:
        ...
        SqlResultQueue * m_queue;
    public:
        SqlQueryHolderEx(SqlQueryHolder *holder, Quad::IQueryCallback * callback, SqlResultQueue * queue)
