...
#ifndef _QUAD_REALM_CONFIG
# define _QUAD_REALM_CONFIG  "QuadRealm.conf"
#endif //_QUAD_REALM_CONFIG

#ifdef WIN32
#include "ServiceWin32.h"
char serviceName[] = "QuadRealm";
char serviceLongName[] = "Quad realm service";
...
extern int main(int argc, char **argv)
{
    ...
    char const* cfg_file = _QUAD_REALM_CONFIG;
    int c=1;
    while(c < argc)
    {
    ...
    {
        sLog.outError("*****************************************************************************");
        sLog.outError(" WARNING: Your quadrealm.conf version indicates your conf file is out of date!");
    ...
    {
        sLog.outError("Quad realm can not bind to %s:%d",bind_ip.c_str(), rmport);
        return 1;
    }
    ...
    #ifdef WIN32
    {
        ...
        if (Prio)
        {
            if (SetPriorityClass(hProcess,HIGH_PRIORITY_CLASS))
                sLog.outString("QuadRealm process priority class set to HIGH");
            else
