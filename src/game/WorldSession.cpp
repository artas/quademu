...
{
    char const* format = GetQuadString(string_id);
    if(format)
    {
        va_list ap;
        char szStr [1024];
...
const char * WorldSession::GetQuadString( int32 entry ) const
{
    return objmgr.GetQuadString(entry,GetSessionDbLocaleIndex());
}
