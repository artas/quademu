        ...
        if (eventId == obj->GetGOInfo()->building.damagedEvent)
        {
            ...
            switch(state->type)
            {
                case BUILDING_WORKSHOP:
                    msgStr = fmtstring(objmgr.GetQuadStringForDBCLocale(LANG_BG_WG_WORKSHOP_DAMAGED), msgStr.c_str(), objmgr.GetQuadStringForDBCLocale(getDefenderTeam() == TEAM_ALLIANCE ? LANG_BG_AB_ALLY : LANG_BG_AB_HORDE));
                    sWorld.SendZoneText(ZONE_WINTERGRASP, msgStr.c_str());
                    break;
                case BUILDING_WALL:
                    sWorld.SendZoneText(ZONE_WINTERGRASP, objmgr.GetQuadStringForDBCLocale(LANG_BG_WG_FORTRESS_UNDER_ATTACK));
                    break;
                case BUILDING_TOWER:
                    ++m_towerDamagedCount[state->GetTeam()];
                    msgStr = fmtstring(objmgr.GetQuadStringForDBCLocale(LANG_BG_WG_TOWER_DAMAGED), msgStr.c_str());
                    sWorld.SendZoneText(ZONE_WINTERGRASP, msgStr.c_str());
                    break;
            }
        }
        else if (eventId == obj->GetGOInfo()->building.destroyedEvent)
        {
            ...
            switch(state->type)
            {
                case BUILDING_WORKSHOP:
                    ...
                    msgStr = fmtstring(objmgr.GetQuadStringForDBCLocale(LANG_BG_WG_WORKSHOP_DESTROYED), msgStr.c_str(), objmgr.GetQuadStringForDBCLocale(getDefenderTeam() == TEAM_ALLIANCE ? LANG_BG_AB_ALLY : LANG_BG_AB_HORDE));
                    sWorld.SendZoneText(ZONE_WINTERGRASP, msgStr.c_str());
                    break;
                case BUILDING_WALL:
                    sWorld.SendZoneText(ZONE_WINTERGRASP, objmgr.GetQuadStringForDBCLocale(LANG_BG_WG_FORTRESS_UNDER_ATTACK));
                ...
                case BUILDING_TOWER:
                    ...
                    msgStr = fmtstring(objmgr.GetQuadStringForDBCLocale(LANG_BG_WG_TOWER_DESTROYED), msgStr.c_str());
                    sWorld.SendZoneText(ZONE_WINTERGRASP, msgStr.c_str());
                    break;
            }
    ...
    else
    {
        ...
        if (isWarTime())
        {
            if (m_timer != 1) // 1 = forceStopBattle
                sWorld.SendZoneText(ZONE_WINTERGRASP, fmtstring(objmgr.GetQuadStringForDBCLocale(entry), objmgr.GetQuadStringForDBCLocale(getDefenderTeam() == TEAM_ALLIANCE ? LANG_BG_AB_ALLY : LANG_BG_AB_HORDE)));
            EndBattle();
        }
        else
        {
            if (m_timer != 1) // 1 = forceStartBattle
                sWorld.SendZoneText(ZONE_WINTERGRASP, objmgr.GetQuadStringForDBCLocale(LANG_BG_WG_BATTLE_STARTS));
            StartBattle();
        }
...
{ // Uptime will do all the work
    ...
    {
        m_timer = 1;
        sWorld.SendZoneText(ZONE_WINTERGRASP, objmgr.GetQuadStringForDBCLocale(LANG_BG_WG_BATTLE_FORCE_START));
    }
}
...
{ // Uptime will do all the work.
    ...
    {
        m_timer = 1;
        sWorld.SendZoneText(ZONE_WINTERGRASP, objmgr.GetQuadStringForDBCLocale(LANG_BG_WG_BATTLE_FORCE_STOP));
    }
}
...
void OPvPWintergrasp::forceChangeTeam()
{
    m_changeDefender = true;
    m_timer = 1;
    sWorld.SendZoneText(ZONE_WINTERGRASP, fmtstring(objmgr.GetQuadStringForDBCLocale(LANG_BG_WG_SWITCH_FACTION), objmgr.GetQuadStringForDBCLocale(getAttackerTeam() == TEAM_ALLIANCE ? LANG_BG_AB_ALLY : LANG_BG_AB_HORDE)));
    if (isWarTime())
        forceStartBattle();
    else
        forceStopBattle();
