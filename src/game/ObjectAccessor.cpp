...
ObjectAccessor::RemoveCorpse(Corpse *corpse)
{
    ...
    CellPair cell_pair = Quad::ComputeCellPair(corpse->GetPositionX(), corpse->GetPositionY());
...
/*void ObjectAccessor::UpdateVisibilityForPlayer( Player* player )
{
    ...
    if (player!=viewPoint)
    {
        CellPair pView(Quad::ComputeCellPair(viewPoint->GetPositionX(), viewPoint->GetPositionY()));
        Cell cellView(pView);
