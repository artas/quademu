...
{
    CellPair p = Quad::ComputeCellPair(obj->GetPositionX(), obj->GetPositionY());
...
{
    CellPair pair = Quad::ComputeCellPair(x, y);
    Cell cell(pair);
...
bool Map::Add(Player *player)
{
    ...
    CellPair p = Quad::ComputeCellPair(player->GetPositionX(), player->GetPositionY());
    Cell cell(p);
...
void Map::RelocationNotify()
{
    ...
    for (std::vector<Unit*>::iterator iter = i_unitsToNotify.begin(); iter != i_unitsToNotify.end(); ++iter)
    {
        ...
        if(dist > 10.0f)
        {
            Quad::VisibleChangesNotifier notifier(*unit);
        ...
        {
            Quad::PlayerRelocationNotifier notifier(*((Player*)unit));
        ...
        {
            Quad::CreatureRelocationNotifier notifier(*((Creature*)unit));
...
void Map::Update(const uint32 &t_diff)
{
    ...
    Quad::ObjectUpdater updater(t_diff);
    // for creature
    TypeContainerVisitor<Quad::ObjectUpdater, GridTypeMapContainer  > grid_object_update(updater);
    // for pets
    TypeContainerVisitor<Quad::ObjectUpdater, WorldTypeMapContainer > world_object_update(updater);
    ...
    for (m_mapRefIter = m_mapRefManager.begin(); m_mapRefIter != m_mapRefManager.end(); ++m_mapRefIter)
    {
        ...
        CellPair standing_cell(Quad::ComputeCellPair(plr->GetPositionX(), plr->GetPositionY()));
    ...
    if(!m_activeNonPlayers.empty())
    {
        for (m_activeNonPlayersIter = m_activeNonPlayers.begin(); m_activeNonPlayersIter != m_activeNonPlayers.end(); )
        {
            ...
            CellPair standing_cell(Quad::ComputeCellPair(obj->GetPositionX(), obj->GetPositionY()));
...
Map::PlayerRelocation(Player *player, float x, float y, float z, float orientation)
{
    ...
    CellPair old_val = Quad::ComputeCellPair(player->GetPositionX(), player->GetPositionY());
    CellPair new_val = Quad::ComputeCellPair(x, y);
...
void Map::MoveAllCreaturesInMoveList()
{
    while(!i_creaturesToMove.empty())
    {
        ...
        // calculate cells
        CellPair new_val = Quad::ComputeCellPair(cm.x, cm.y);
        Cell new_cell(new_val);
...
bool Map::CreatureRespawnRelocation(Creature *c)
{
    ...
    CellPair resp_val = Quad::ComputeCellPair(resp_x, resp_y);
...
bool Map::CheckGridIntegrity(Creature* c, bool moved) const
{
    ...
    CellPair xy_val = Quad::ComputeCellPair(c->GetPositionX(), c->GetPositionY());
    Cell xy_cell(xy_val);
    if(xy_cell != cur_cell)
    {
...
void Map::UpdateObjectVisibility( WorldObject* obj, Cell cell, CellPair cellpair)
{
    cell.data.Part.reserved = ALL_DISTRICT;
    cell.SetNoCreate();
    Quad::VisibleChangesNotifier notifier(*obj);
    TypeContainerVisitor<Quad::VisibleChangesNotifier, WorldTypeMapContainer > player_notifier(notifier);
...
bool Map::ActiveObjectsNearGrid(uint32 x, uint32 y) const
{
    ...
    for (MapRefManager::const_iterator iter = m_mapRefManager.begin(); iter != m_mapRefManager.end(); ++iter)
    {
        ...
        CellPair p = Quad::ComputeCellPair(plr->GetPositionX(), plr->GetPositionY());
...
void Map::AddToActive( Creature* c )
{
    ...
    if(!c->isPet() && c->GetDBTableGUIDLow())
    {
        ...
        GridPair p = Quad::ComputeGridPair(x, y);
        if(getNGrid(p.x_coord, p.y_coord))
            getNGrid(p.x_coord, p.y_coord)->incUnloadActiveLock();
        else
        {
            GridPair p2 = Quad::ComputeGridPair(c->GetPositionX(), c->GetPositionY());
    ...
    while (!m_scriptSchedule.empty() && (iter->first <= sWorld.GetGameTime()))
    {
        ...
        switch (step.script->command)
        {
            ...
            case SCRIPT_COMMAND_CALLSCRIPT_TO_UNIT:
            {
                ...
                {
                    CellPair p(Quad::ComputeCellPair(((Unit*)source)->GetPositionX(), ((Unit*)source)->GetPositionY()));
                    Cell cell(p);
                    cell.data.Part.reserved = ALL_DISTRICT;

                    //sLog.outDebug("Attempting to find Creature: Db GUID: %i", step.script->datalong);
                    Quad::CreatureWithDbGUIDCheck target_check(((Unit*)source), step.script->datalong);
                    Quad::CreatureSearcher<Quad::CreatureWithDbGUIDCheck> checker(((Unit*)source), target, target_check);

                    TypeContainerVisitor<Quad::CreatureSearcher <Quad::CreatureWithDbGUIDCheck>, GridTypeMapContainer > unit_checker(checker);
                    CellLock<GridReadGuard> cell_lock(cell, p);
