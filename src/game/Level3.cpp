...
bool ChatHandler::HandleReloadQuadStringCommand(const char*)
{
    sLog.outString( "Re-Loading quad_string Table!" );
    objmgr.LoadQuadStrings();
    SendGlobalGMSysMessage("DB table `quad_string` reloaded.");
    return true;
}
...
bool ChatHandler::HandleAddItemCommand(const char* args)
{
    ...
    sLog.outDetail(GetQuadString(LANG_ADDITEM), itemId, count);
...
bool ChatHandler::HandleAddItemSetCommand(const char* args)
{
    ...
    sLog.outDetail(GetQuadString(LANG_ADDITEMSET), itemsetId);
    ...
    for (uint32 id = 0; id < sSkillLineStore.GetNumRows(); id++)
    {
        ...
        if(skillInfo)
        {
            ...
            if(loc < MAX_LOCALE)
            {
                ...
                {
                    knownStr = GetQuadString(LANG_KNOWN);
                    uint32 curValue = target->GetPureSkillValue(id);
                    uint32 maxValue  = target->GetPureMaxSkillValue(id);
                    uint32 permValue = target->GetSkillPermBonusValue(id);
                    uint32 tempValue = target->GetSkillTempBonusValue(id);

                    char const* valFormat = GetQuadString(LANG_SKILL_VALUES);
    ...
    for (uint32 id = 0; id < sSpellStore.GetNumRows(); id++)
    {
        ...
        if(spellInfo)
        {
            ...
            if(loc < MAX_LOCALE)
            {
                ...
                if(rank)
                    ss << GetQuadString(LANG_SPELL_RANK) << rank;
                ...
                if(talent)
                    ss << GetQuadString(LANG_TALENT);
                if(passive)
                    ss << GetQuadString(LANG_PASSIVE);
                if(learn)
                    ss << GetQuadString(LANG_LEARN);
                if(known)
                    ss << GetQuadString(LANG_KNOWN);
                if(active)
                    ss << GetQuadString(LANG_ACTIVE);
                        ...
                        if(target)
                        {
                            ...
                            if(status == QUEST_STATUS_COMPLETE)
                            {
                                if(target->GetQuestRewardStatus(qinfo->GetQuestId()))
                                    statusStr = GetQuadString(LANG_COMMAND_QUEST_REWARDED);
                                else
                                    statusStr = GetQuadString(LANG_COMMAND_QUEST_COMPLETE);
                            }
                            else if(status == QUEST_STATUS_INCOMPLETE)
                                statusStr = GetQuadString(LANG_COMMAND_QUEST_ACTIVE);
                        }
            ...
            if(loc < MAX_LOCALE)
            {
                ...
                if (repState)                               // and then target!=NULL also
                {
                    ...
                    if(repState->Flags & FACTION_FLAG_VISIBLE)
                        ss << GetQuadString(LANG_FACTION_VISIBLE);
                    if(repState->Flags & FACTION_FLAG_AT_WAR)
                        ss << GetQuadString(LANG_FACTION_ATWAR);
                    if(repState->Flags & FACTION_FLAG_PEACE_FORCED)
                        ss << GetQuadString(LANG_FACTION_PEACE_FORCED);
                    if(repState->Flags & FACTION_FLAG_HIDDEN)
                        ss << GetQuadString(LANG_FACTION_HIDDEN);
                    if(repState->Flags & FACTION_FLAG_INVISIBLE_FORCED)
                        ss << GetQuadString(LANG_FACTION_INVISIBLE_FORCED);
                    if(repState->Flags & FACTION_FLAG_INACTIVE)
                        ss << GetQuadString(LANG_FACTION_INACTIVE);
                }
                else
                    ss << GetQuadString(LANG_FACTION_NOREPUTATION);
        ...
        if(MapInfo)
        {
            ...
            if(loc < MAX_LOCALE)
            {
                ...
                if(MapInfo->IsContinent())
                    ss << GetQuadString(LANG_CONTINENT);

                switch(MapInfo->map_type)
                {
                    case MAP_INSTANCE:      ss << GetQuadString(LANG_INSTANCE);      break;
                    case MAP_BATTLEGROUND:  ss << GetQuadString(LANG_BATTLEGROUND);  break;
                    case MAP_ARENA:         ss << GetQuadString(LANG_ARENA);         break;
                }

                if(MapInfo->IsRaid())
                    ss << GetQuadString(LANG_RAID);

                if(MapInfo->SupportsHeroicMode())
                    ss << GetQuadString(LANG_HEROIC);
                ...
                if(MapInfo->IsMountAllowed())
                    ss << GetQuadString(LANG_MOUNTABLE);
...
bool ChatHandler::HandleNearGraveCommand(const char* args)
{
    ...
    if(graveyard)
    {
        ...
        std::string team_name = GetQuadString(LANG_COMMAND_GRAVEYARD_NOTEAM);

        if(g_team == 0)
            team_name = GetQuadString(LANG_COMMAND_GRAVEYARD_ANY);
        else if(g_team == HORDE)
            team_name = GetQuadString(LANG_COMMAND_GRAVEYARD_HORDE);
        else if(g_team == ALLIANCE)
            team_name = GetQuadString(LANG_COMMAND_GRAVEYARD_ALLIANCE);
...
bool ChatHandler::HandleDebugSet32Bit(const char* args)
{
    ...
    sLog.outDebug(GetQuadString(LANG_SET_32BIT), Opcode, Value);
...
bool ChatHandler::HandleListAurasCommand (const char * /*args*/)
{
    ...
    char const* talentStr = GetQuadString(LANG_TALENT);
    char const* passiveStr = GetQuadString(LANG_PASSIVE);
...
bool ChatHandler::HandleBanInfoHelper(uint32 accountid, char const* accountname)
{
    ...
    do
    {
        ...
        std::string bantime = permanent?GetQuadString(LANG_BANINFO_INFINITE):secsToTimeString(fields[1].GetUInt64(), true);
        PSendSysMessage(LANG_BANINFO_HISTORYENTRY,
            fields[0].GetString(), bantime.c_str(), active ? GetQuadString(LANG_BANINFO_YES):GetQuadString(LANG_BANINFO_NO), fields[4].GetString(), fields[5].GetString());
    }while (result->NextRow());
...
bool ChatHandler::HandleBanInfoIPCommand(const char* args)
{
    ...
    PSendSysMessage(LANG_BANINFO_IPENTRY,
        fields[0].GetString(), fields[1].GetString(), permanent ? GetQuadString(LANG_BANINFO_NEVER):fields[2].GetString(),
        permanent ? GetQuadString(LANG_BANINFO_INFINITE):secsToTimeString(fields[3].GetUInt64(), true).c_str(), fields[4].GetString(), fields[5].GetString());
    delete result;
    return true;
}
...
bool ChatHandler::HandleRespawnCommand(const char* /*args*/)
{
    ...
    CellPair p(Quad::ComputeCellPair(pl->GetPositionX(), pl->GetPositionY()));
    Cell cell(p);
    ...
    TypeContainerVisitor<Quad::WorldObjectWorker<Quad::RespawnDo>, GridTypeMapContainer > obj_worker(worker);
