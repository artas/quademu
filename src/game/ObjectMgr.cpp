    ...
    // Loading levels data
    {
        ...
        do
        {
            ...
            if(current_level > sWorld.getConfig(CONFIG_MAX_PLAYER_LEVEL))
            {
                ...
                {
                    sLog.outDetail("Unused (> MaxPlayerLevel in Quad.conf) level %u in `pet_levelstats` table, ignoring.",current_level);
    ...
    // Loading levels data (class only dependent)
    {
        ...
        do
        {
            ...
            if(current_level > sWorld.getConfig(CONFIG_MAX_PLAYER_LEVEL))
            {
                ...
                {
                    sLog.outDetail("Unused (> MaxPlayerLevel in Quad.conf) level %u in `player_classlevelstats` table, ignoring.",current_level);
                    ++count;                                // make result loading percent "expected" correct in case disabled detail mode for example.
                }
                continue;
            }
    ...
    // Loading levels data (class/race dependent)
    {
        ...
        do
        {
            ...
            if(current_level > sWorld.getConfig(CONFIG_MAX_PLAYER_LEVEL))
            {
                ...
                {
                    sLog.outDetail("Unused (> MaxPlayerLevel in Quad.conf) level %u in `player_levelstats` table, ignoring.",current_level);
                    ++count;                                // make result loading percent "expected" correct in case disabled detail mode for example.
                }
                continue;
            }
