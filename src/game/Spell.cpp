...
void Spell::SearchAreaTarget(std::list<Unit*> &TagUnitMap, float radius, SpellNotifyPushType type, SpellTargets TargetType, uint32 entry)
{
    ...
    Quad::SpellNotifierCreatureAndPlayer notifier(m_caster, TagUnitMap, radius, type, TargetType, pos, entry);
...
WorldObject* Spell::SearchNearbyTarget(float range, SpellTargets TargetType)
{
    switch(TargetType)
    {
        ...
        {
            Unit *target = NULL;
            Quad::AnyUnfriendlyUnitInObjectRangeCheck u_check(m_caster, m_caster, range);
            Quad::UnitLastSearcher<Quad::AnyUnfriendlyUnitInObjectRangeCheck> searcher(m_caster, target, u_check);
        ...
        {
            Unit *target = NULL;
            Quad::AnyFriendlyUnitInObjectRangeCheck u_check(m_caster, m_caster, range);
            Quad::UnitLastSearcher<Quad::AnyFriendlyUnitInObjectRangeCheck> searcher(m_caster, target, u_check);
            m_caster->VisitNearbyObject(range, searcher);
            return target;
        }
    }
    ...
    else if (pushType)
    {
                ...
                switch (m_spellInfo->Id)
                {
                    ...
                    case 51328:
                                ...
                                && m_targets.getUnitTarget()->GetDisplayId() == m_targets.getUnitTarget()->GetNativeDisplayId())))
                        {
                            ...
                            WorldObject* result = FindCorpseUsing <Quad::ExplodeCorpseObjectCheck> ();
        ...
        else
        {
            switch (cur)
            {
                ...
                case TARGET_OBJECT_AREA_DST:
                {
                    ...
                    Quad::GameObjectInRangeCheck check(x, y, z, radius + 50);
                    std::list<GameObject*> goList;
                    Quad::GameObjectListSearcher<Quad::GameObjectInRangeCheck> searcher(m_caster, goList, check);
        ...
        if(!unitList.empty())
        {
            if(uint32 maxTargets = m_spellValue->MaxAffectedTargets)
            {
                ...
                Quad::RandomResizeList(unitList, m_spellValue->MaxAffectedTargets);
            }
            else
            {
                switch (m_spellInfo->Id)
...
SpellCastResult Spell::CheckItems()
{
    ...
    {
        CellPair p(Quad::ComputeCellPair(m_caster->GetPositionX(), m_caster->GetPositionY()));
        Cell cell(p);
        cell.data.Part.reserved = ALL_DISTRICT;
        ...
        TypeContainerVisitor<Quad::GameObjectSearcher<Quad::GameObjectFocusCheck>, GridTypeMapContainer > object_checker(checker);
        CellLock<GridReadGuard> cell_lock(cell, p);
