 ...
 */
#ifndef QUAD_SPELLAURAS_H
#define QUAD_SPELLAURAS_H
...
//
// Other case choice: each code line moved under if(Real) check is Quad speedup,
//      each setting object update field code line moved under if(Real) check is significant Quad speedup, and less server->client data sends
//      each packet sending code moved under if(Real) check is _large_ Quad speedup, and lot less server->client data sends
//
...
class QUAD_DLL_SPEC AuraEffect
{
    public:
...
class QUAD_DLL_SPEC AreaAuraEffect : public AuraEffect
{
    public:
        friend AuraEffect* CreateAuraEffect(Aura * parentAura, uint32 effIndex, int32 *currentBasePoints);
        void Update(uint32 diff);
...
class QUAD_DLL_SPEC PersistentAreaAuraEffect : public AuraEffect
{
    public:
        friend AuraEffect* CreateAuraEffect(Aura * parentAura, uint32 effIndex, int32 *currentBasePoints);
        void Update(uint32 diff);
