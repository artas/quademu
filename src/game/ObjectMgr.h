...
typedef UNORDERED_MAP<int32,QuadStringLocale> QuadStringLocaleMap;
...
struct QUAD_DLL_SPEC LanguageDesc
{
    Language lang_id;
    uint32   spell_id;
    uint32   skill_id;
...
QUAD_DLL_SPEC LanguageDesc const* GetLanguageDescByID(uint32 lang);
...
class ObjectMgr
{
    ...
    public:
        ...
        bool LoadQuadStrings(DatabaseType& db, char const* table, int32 min_value, int32 max_value);
        bool LoadQuadStrings() { return LoadQuadStrings(WorldDatabase,"quad_string",MIN_QUAD_STRING_ID,MAX_QUAD_STRING_ID); }
        ...
        QuadStringLocale const* GetQuadStringLocale(int32 entry) const
        {
            QuadStringLocaleMap::const_iterator itr = mQuadStringLocaleMap.find(entry);
            if(itr==mQuadStringLocaleMap.end()) return NULL;
            return &itr->second;
        }
        const char *GetQuadString(int32 entry, int locale_idx) const;
        const char *GetQuadStringForDBCLocale(int32 entry) const { return GetQuadString(entry,DBCLocaleIndex); }
...
//        GraveYardMap        mGraveYardMap;
    ...
    private:
        ...
        QuadStringLocaleMap mQuadStringLocaleMap;
...
#define objmgr Quad::Singleton<ObjectMgr>::Instance()

// scripting access functions
QUAD_DLL_SPEC bool LoadQuadStrings(DatabaseType& db, char const* table,int32 start_value = MAX_CREATURE_AI_TEXT_STRING_ID, int32 end_value = std::numeric_limits<int32>::min());
QUAD_DLL_SPEC uint32 GetAreaTriggerScriptId(uint32 trigger_id);
QUAD_DLL_SPEC uint32 GetScriptId(const char *name);
QUAD_DLL_SPEC ObjectMgr::ScriptNameMap& GetScriptNames();
QUAD_DLL_SPEC GameObjectInfo const *GetGameObjectInfo(uint32 id);
QUAD_DLL_SPEC CreatureInfo const *GetCreatureInfo(uint32 id);
QUAD_DLL_SPEC CreatureInfo const* GetCreatureTemplateStore(uint32 entry);
QUAD_DLL_SPEC Quest const* GetQuestTemplateStore(uint32 entry);
