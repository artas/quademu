...
#ifndef QUAD_GRIDNOTIFIERSIMPL_H
#define QUAD_GRIDNOTIFIERSIMPL_H
...
inline void
Quad::ObjectUpdater::Visit(CreatureMapType &m)
{
...
template<class T>
inline void
Quad::PlayerVisibilityNotifier::Visit(GridRefManager<T> &m)
{
...
template<>
inline void
Quad::PlayerRelocationNotifier::Visit(PlayerMapType &m)
{
...
template<>
inline void
Quad::PlayerRelocationNotifier::Visit(CreatureMapType &m)
{
    for (CreatureMapType::iterator iter = m.begin(); iter != m.end(); ++iter)
    {
        i_clientGUIDs.erase(iter->getSource()->GetGUID());
...
template<>
inline void
Quad::CreatureRelocationNotifier::Visit(PlayerMapType &m)
{
    for (PlayerMapType::iterator iter = m.begin(); iter != m.end(); ++iter)
    {
        if(iter->getSource()->m_Notified)
...
template<>
inline void
Quad::CreatureRelocationNotifier::Visit(CreatureMapType &m)
{
...
inline void Quad::DynamicObjectUpdater::VisitHelper(Unit* target)
{
...
template<>
inline void
Quad::DynamicObjectUpdater::Visit(CreatureMapType  &m)
{
...
template<>
inline void
Quad::DynamicObjectUpdater::Visit(PlayerMapType  &m)
{
...
void Quad::WorldObjectSearcher<Check>::Visit(GameObjectMapType &m)
{
    // already found
    if(i_object)
        return;
...
template<class Check>
void Quad::WorldObjectSearcher<Check>::Visit(PlayerMapType &m)
{
    // already found
    if(i_object)
        return;
...
template<class Check>
void Quad::WorldObjectSearcher<Check>::Visit(CreatureMapType &m)
{
    // already found
    if(i_object)
        return;
...
template<class Check>
void Quad::WorldObjectSearcher<Check>::Visit(CorpseMapType &m)
{
    // already found
    if(i_object)
        return;
...
template<class Check>
void Quad::WorldObjectSearcher<Check>::Visit(DynamicObjectMapType &m)
{
    // already found
    if(i_object)
        return;
...
template<class Check>
void Quad::WorldObjectListSearcher<Check>::Visit(PlayerMapType &m)
{
    for (PlayerMapType::iterator itr=m.begin(); itr != m.end(); ++itr)
        if(itr->getSource()->InSamePhase(i_phaseMask))
            if(i_check(itr->getSource()))
...
template<class Check>
void Quad::WorldObjectListSearcher<Check>::Visit(CreatureMapType &m)
{
    for (CreatureMapType::iterator itr=m.begin(); itr != m.end(); ++itr)
        if(itr->getSource()->InSamePhase(i_phaseMask))
            if(i_check(itr->getSource()))
...
template<class Check>
void Quad::WorldObjectListSearcher<Check>::Visit(CorpseMapType &m)
{
    for (CorpseMapType::iterator itr=m.begin(); itr != m.end(); ++itr)
        if(itr->getSource()->InSamePhase(i_phaseMask))
            if(i_check(itr->getSource()))
...
template<class Check>
void Quad::WorldObjectListSearcher<Check>::Visit(GameObjectMapType &m)
{
    for (GameObjectMapType::iterator itr=m.begin(); itr != m.end(); ++itr)
        if(itr->getSource()->InSamePhase(i_phaseMask))
            if(i_check(itr->getSource()))
...
template<class Check>
void Quad::WorldObjectListSearcher<Check>::Visit(DynamicObjectMapType &m)
{
    for (DynamicObjectMapType::iterator itr=m.begin(); itr != m.end(); ++itr)
        if(itr->getSource()->InSamePhase(i_phaseMask))
            if(i_check(itr->getSource()))
...
template<class Check>
void Quad::GameObjectSearcher<Check>::Visit(GameObjectMapType &m)
{
    // already found
    if(i_object)
        return;
...
template<class Check>
void Quad::GameObjectLastSearcher<Check>::Visit(GameObjectMapType &m)
{
    for (GameObjectMapType::iterator itr=m.begin(); itr != m.end(); ++itr)
    {
        if(!itr->getSource()->InSamePhase(i_phaseMask))
...
template<class Check>
void Quad::GameObjectListSearcher<Check>::Visit(GameObjectMapType &m)
{
    for (GameObjectMapType::iterator itr=m.begin(); itr != m.end(); ++itr)
        if(itr->getSource()->InSamePhase(i_phaseMask))
            if(i_check(itr->getSource()))
...
template<class Check>
void Quad::UnitSearcher<Check>::Visit(CreatureMapType &m)
{
    // already found
    if(i_object)
        return;
...
template<class Check>
void Quad::UnitSearcher<Check>::Visit(PlayerMapType &m)
{
    // already found
    if(i_object)
        return;
...
template<class Check>
void Quad::UnitLastSearcher<Check>::Visit(CreatureMapType &m)
{
    for (CreatureMapType::iterator itr=m.begin(); itr != m.end(); ++itr)
    {
        if(!itr->getSource()->InSamePhase(i_phaseMask))
...
template<class Check>
void Quad::UnitLastSearcher<Check>::Visit(PlayerMapType &m)
{
    for (PlayerMapType::iterator itr=m.begin(); itr != m.end(); ++itr)
    {
        if(!itr->getSource()->InSamePhase(i_phaseMask))
...
template<class Check>
void Quad::UnitListSearcher<Check>::Visit(PlayerMapType &m)
{
    for (PlayerMapType::iterator itr=m.begin(); itr != m.end(); ++itr)
        if(itr->getSource()->InSamePhase(i_phaseMask))
            if(i_check(itr->getSource()))
...
template<class Check>
void Quad::UnitListSearcher<Check>::Visit(CreatureMapType &m)
{
    for (CreatureMapType::iterator itr=m.begin(); itr != m.end(); ++itr)
        if(itr->getSource()->InSamePhase(i_phaseMask))
            if(i_check(itr->getSource()))
...
template<class Check>
void Quad::CreatureSearcher<Check>::Visit(CreatureMapType &m)
{
    // already found
    if(i_object)
        return;
...
template<class Check>
void Quad::CreatureLastSearcher<Check>::Visit(CreatureMapType &m)
{
    for (CreatureMapType::iterator itr=m.begin(); itr != m.end(); ++itr)
    {
        if(!itr->getSource()->InSamePhase(i_phaseMask))
...
template<class Check>
void Quad::CreatureListSearcher<Check>::Visit(CreatureMapType &m)
{
    for (CreatureMapType::iterator itr=m.begin(); itr != m.end(); ++itr)
        if(itr->getSource()->InSamePhase(i_phaseMask))
            if( i_check(itr->getSource()))
...
template<class Check>
void Quad::PlayerListSearcher<Check>::Visit(PlayerMapType &m)
{
    for (PlayerMapType::iterator itr=m.begin(); itr != m.end(); ++itr)
        if(itr->getSource()->InSamePhase(i_phaseMask))
            if( i_check(itr->getSource()))
...
template<class Check>
void Quad::PlayerSearcher<Check>::Visit(PlayerMapType &m)
{
    // already found
    if(i_object)
        return;
