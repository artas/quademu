...
Unit* Creature::SelectNearestTarget(float dist) const
{
    ...
    {
        Quad::NearestHostileUnitInAttackDistanceCheck u_check(this, dist);
        Quad::UnitLastSearcher<Quad::NearestHostileUnitInAttackDistanceCheck> searcher(this, target, u_check);

        TypeContainerVisitor<Quad::UnitLastSearcher<Quad::NearestHostileUnitInAttackDistanceCheck>, WorldTypeMapContainer > world_unit_searcher(searcher);
        TypeContainerVisitor<Quad::UnitLastSearcher<Quad::NearestHostileUnitInAttackDistanceCheck>, GridTypeMapContainer >  grid_unit_searcher(searcher);
    ...
    if( !m_AlreadyCallAssistance && getVictim() && !isPet() && !isCharmed())
    {
        ...
        if(radius > 0)
        {
...

            {
                ...
                TypeContainerVisitor<Quad::CreatureListSearcher<Quad::AnyAssistCreatureInRangeCheck>, GridTypeMapContainer >  grid_creature_searcher(searcher);
