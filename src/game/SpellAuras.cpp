    ...
    if(GetParentAura()->GetSourceGUID() == m_target->GetGUID())
    {
        ...
        if( !source->hasUnitState(UNIT_STAT_ISOLATED) )
        {
            ...
            switch(m_areaAuraType)
            {
                ...
                {
                    Quad::AnyFriendlyUnitInObjectRangeCheck u_check(source, caster, m_radius);
                    Quad::UnitListSearcher<Quad::AnyFriendlyUnitInObjectRangeCheck> searcher(source, targets, u_check);
                    source->VisitNearbyObject(m_radius, searcher);
                    break;
                }
                case AREA_AURA_ENEMY:
                {
                    Quad::AnyAoETargetUnitInObjectRangeCheck u_check(source, caster, m_radius); // No GetCharmer in searcher
                    Quad::UnitListSearcher<Quad::AnyAoETargetUnitInObjectRangeCheck> searcher(source, targets, u_check);
                    source->VisitNearbyObject(m_radius, searcher);
                    break;
                }
...
void AuraEffect::HandleFeignDeath(bool apply, bool Real, bool /*changeAmount*/)
{
    ...
    if( apply )
    {
        ...
        */
        ...
        std::list<Unit*> targets;
        Quad::AnyUnfriendlyUnitInObjectRangeCheck u_check(m_target, m_target, m_target->GetMap()->GetVisibilityDistance());
        Quad::UnitListSearcher<Quad::AnyUnfriendlyUnitInObjectRangeCheck> searcher(m_target, targets, u_check);
