...
struct RGuard
{
    ...
    Quad::GeneralLock<LOCK_TYPE> i_lock;
};
...
class CORE_DLL_SPEC Map : public GridRefManager<NGridType>, public MaNGOS::ObjectLevelLockable<Map, ACE_Thread_Mutex>
{
...
class QUAD_DLL_SPEC InstanceMap : public Map
{
    public:
...
class QUAD_DLL_SPEC BattleGroundMap : public Map
{
    public:
...
{
    float x_off, y_off;
    CellPair p(Quad::ComputeCellPair(x, y, x_off, y_off));
    Cell cell(p);
